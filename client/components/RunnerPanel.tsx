...
type RunEventData = Partial<RunProgress & LastRunState> & {
  running?: boolean;
};
  ...
  return (
    <div className="flex flex-col gap-3 rounded-2xl border border-zinc-200 bg-white p-5 shadow-sm">
      <div className="flex items-center justify-between">
        <div>
          <p className="text-sm font-semibold text-zinc-950">Scraper Engine</p>
      ...
      {running && progress && (
        <div className="space-y-1.5">
          ...
          <div className="flex items-center gap-4 text-[11px] text-zinc-500">
            <span className="text-emerald-600">{progress.changed} changed</span>
            {progress.errors > 0 && (
              <div className="flex items-center gap-1.5">
                <span className="text-red-600">{progress.errors} errors</span>
                <button
                  onClick={() => setShowErrors(!showErrors)}
                  className="rounded bg-zinc-100 px-1.5 py-0.5 text-[9px] text-zinc-700 hover:bg-zinc-200"
                >
      ...
      {!running && lastRun && (
        <div className="flex items-center gap-4 text-[11px] text-zinc-500">
          <span>
            Last run — {lastRun.checked} checked, <span className="text-emerald-600">{lastRun.changed} changed</span>
          </span>
          {lastRun.errors > 0 && (
            <div className="flex items-center gap-1.5">
              <span className="text-red-600">{lastRun.errors} errors</span>
              <button
                onClick={() => setShowErrors(!showErrors)}
                className="rounded bg-zinc-100 px-1.5 py-0.5 text-[9px] text-zinc-700 hover:bg-zinc-200"
              >
      ...
      {showErrors && currentErrors.length > 0 && (
        <div className="mt-2 max-h-40 overflow-y-auto rounded-xl border border-red-200 bg-red-50 p-3 text-[10px] font-mono leading-relaxed text-red-700">
          <div className="mb-1.5 font-bold uppercase tracking-wider text-red-600">Scraper Error Log:</div>
          <ul className="space-y-1">
            {currentErrors.map((err, idx) => (
              <li key={idx} className="border-b border-red-100 pb-1 last:border-0 last:pb-0">
                <span className="font-semibold text-red-800">[{err.name}]</span>: {err.error}
              </li>
            ))}
          </ul>
        </div>
